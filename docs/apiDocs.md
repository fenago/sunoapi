...
### Bring Your Own API Key (UI-Only)

- Users of the Resonance demo can now supply their own Suno API key directly in the app interface.
- Visit [sunoapi.org](https://sunoapi.org/) to create an account and generate a personal API key.
- Paste the key into the **Suno API Key** box on the homepage and press **Save Key**. The key stays in your browser storage only.
- Once saved, all generate, status, and credit checks run against your key so you control billing.
- Environment variables like `SUNOAPI_ORG` are no longer used; without adding a key through the UI requests will fail.
